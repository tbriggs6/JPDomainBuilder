...
public class IntersectionPerformance {
	...
	public IntersectionPerformance(int nclasses, int nprop, int nindiv, int maxdisjunction) throws OntologyLoadException
	{
	...
	private void constructModel( )
	{
		...
		for (int i = 0; i < nproperties; i++)
		{
			...
			OWLIntersectionClass ouc = owlModel.createOWLIntersectionClass();
	...
	{
		try {
			PrintWriter out = new PrintWriter(new FileOutputStream("intersection-results.txt"));
			for (int classes = 20; classes < 300; classes+=10) {
				
				IntersectionPerformance DP = new IntersectionPerformance( classes,classes/2, classes*2,(int) Math.sqrt(classes));
				DP.write();
				long time = DP.reason();
